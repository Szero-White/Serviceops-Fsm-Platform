...
export function AppLayout() {
  ...
  const { data: unread = 0 } = useQuery({ queryKey: ['notification-count'], queryFn: notificationsApi.unreadCount, refetchInterval: 30_000 })
  const { data: notifications } = useQuery({ queryKey: ['notifications'], queryFn: notificationsApi.list, enabled: notificationsOpen })
    ...
    <Layout className="app-shell">
      <Sider trigger={null} collapsible collapsed={collapsed} width={258} className="app-sider">
        <div className="brand">
          ...
          {!collapsed && <div><strong>ServiceOps</strong><span>Field Service SaaS</span></div>}
        </div>
        <Menu theme="dark" mode="inline" selectedKeys={[selectedKey]} items={items} className="app-menu" />
        {!collapsed && (
          <div className="sider-support">
            <ToolOutlined />
            <div><strong>Local-first MVP</strong><span>Phiên bản chạy nội bộ</span></div>
          </div>
        )}
      </Sider>
      <Layout>
        <Header className="app-header">
          ...
          >
            <button className="user-menu">
              ...
              <span><strong>{user?.displayName}</strong><small>{roleLabels[user?.role ?? ''] ?? user?.role}</small></span>
            </button>
          </Dropdown>
        </Header>
      ...
      <Drawer title="Thông báo" open={notificationsOpen} onClose={() => setNotificationsOpen(false)} width={420}>
            ...
            renderItem={(item) => (
              ...
              >
                <List.Item.Meta
                  ...
                  title={item.title}
                  description={<Space direction="vertical" size={2}><Typography.Text>{item.message}</Typography.Text><Typography.Text type="secondary">{formatDateTime(item.createdAt)}</Typography.Text></Space>}
                />
              </List.Item>
            )}
          />
