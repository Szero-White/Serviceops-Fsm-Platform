...
- `CUSTOMER_SERVICE`: có thể `REOPENED`/`CANCELLED` theo policy trước customer acceptance; cả reopen và cancel đều bắt buộc nhập lý do để giữ status history/notification có ngữ cảnh. Sau `SETTLED` dùng action closure riêng để chuyển `CUSTOMER_ACCEPTED → CLOSED`.
...
- Locking + validation + transaction ngăn stock âm, double issue và serialize thay đổi cùng một phụ tùng.
...
- Inventory movement history snapshots actor name/role cho mọi stock movement; riêng `ISSUE` còn snapshot kỹ thuật viên nhận hàng để ledger trả lời rõ **ai cấp** và **ai nhận** ngay cả khi Work Order được điều phối lại sau đó. Mục đích sử dụng nằm ở part request/actual-used của Work Order; `CONSUME` chỉ còn compatibility lịch sử.
...
- Copy runtime đi qua public facade `NotificationCopy`; wording chuyên biệt được tách vào các collaborator package-private theo Work Order dispatch, payment và inventory. Service nghiệp vụ và `AppLayout` không tự ghép title/message. `V7__notification_feed_cleanup.sql` loại khỏi bell các row CRUD/import/generic-status cũ vốn đã có Audit/Timeline làm nguồn truy vết, nhờ đó unread count không còn tính cả noise. Các actionable legacy assignment/reschedule/reopen/cancel còn lại được normalize riêng tại `features/notifications/presentation.ts`.
