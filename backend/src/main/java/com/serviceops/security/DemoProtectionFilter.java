...
public class DemoProtectionFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        body.put("detail", "Public demo đang bảo vệ thao tác quản trị hoặc xóa dữ liệu. Chức năng vẫn khả dụng khi DEMO_MODE=false.");
    ...
    private static boolean isBlockedMutation(HttpServletRequest request) {
        ...
        if (matchesPathOrChild(path, "/api/v1/users")
                || matchesPathOrChild(path, "/api/v1/technicians")
                || matchesPathOrChild(path, "/api/v1/service-channels")) {
    ...
    private static boolean matchesPathOrChild(String requestPath, String protectedPath) {
        return requestPath.equals(protectedPath) || requestPath.startsWith(protectedPath + "/");
    }
}
