...
class DemoProtectionFilterTest {
    ...
    void blocksDeleteInDemoMode() throws Exception {
        ...
        assertThat(fixture.response().getContentAsString()).contains("DEMO_WRITE_PROTECTED");
        verifyNoInteractions(fixture.chain());
    }

    @Test
    void blocksUserAdministrationWritesInDemoMode() throws Exception {
    ...
    @Test
    void blocksTechnicianAndServiceChannelAdministrationWritesInDemoMode() throws Exception {
        var technician = fixture(true, "POST", "/api/v1/technicians");
        var channel = fixture(true, "PUT", "/api/v1/service-channels/123");

        technician.filter().doFilter(technician.request(), technician.response(), technician.chain());
        channel.filter().doFilter(channel.request(), channel.response(), channel.chain());

        assertThat(technician.response().getStatus()).isEqualTo(403);
        assertThat(channel.response().getStatus()).isEqualTo(403);
    }
    ...
    void keepsCoreWorkflowWritesAvailableInDemoMode() throws Exception {
        for (String path : new String[]{"/api/v1/customers", "/api/v1/service-requests", "/api/v1/work-orders", "/api/v1/spare-parts"}) {
    ...
    private static Fixture fixture(boolean enabled, String method, String path) {
        DemoProtectionFilter filter = new DemoProtectionFilter(new DemoProperties(enabled, "demo-password"), new ObjectMapper().findAndRegisterModules());
    ...
    private record Fixture(DemoProtectionFilter filter,
